...
const JobCard: React.FC<JobCardProps> = ({ job }) => {
  const { user, btnLoading, applyToJob, applications } = useAppData();
  ...
  useEffect(() => {
    if (applications && job.job_id) {
      applications.forEach((application: any) => {
        if (application.job_id === job.job_id) setApplied(true);
      });
    }
      ...
      <CardContent className="flex flex-col gap-3 pt-4 border-t">
        <div className="flex w-full gap-2">
          ...
          {user && user.role === "jobseeker" && (
            <>
              {applied ? (
                <div className="flex-1 flex items-center justify-center gap-2 text-green-500 font-medium text-sm bg-green-100 dark:bg-green-900 rounded-md px-3 py-2">
